...
function DixitPlayContent() {
    ...
    const handlePing = () => {
        if (socket) {
            ...
            socket.emit('dixit:ping', {}, (res: any) => {
                console.log('PING response:', res);
                alert('Server says: ' + (res?.message || 'No message (response received)'));
            });
        } else {
            alert('Socket není připojen.');
        }
    };
    ...
    const handleCreate = () => {
        if (socket && nickname) {
            console.log('Initiating dixit:create', { nickname, avatar, socketId: socket.id });
            const toastId = setTimeout(() => alert('Hra se zakládá delší dobu, prosím čekejte...'), 2000);

            // Timeout promise
            const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout - Server neodpověděl do 10s')), 10000));

            // Emit promise
            const emit = new Promise((resolve, reject) => {
                socket.emit('dixit:create', { guestInfo: { nickname, avatar } }, (response: any) => {
                    clearTimeout(toastId);
                    console.log('Response from dixit:create', response);
                    if (response.success && response.playerId && response.game) {
                        resolve(response);
                    } else {
                        reject(new Error(response.error || 'Neznámá chyba serveru'));
                    }
                });
            });
            ...
            Promise.race([emit, timeout])
                ...
                .catch(err => {
                    ...
                    alert('Chyba při zakládání: ' + err.message);
                });
        } else {
            alert('Chyba: Socket nepřipojen nebo chybí jméno.');
        }
    };
                    ...
                    {(mode === 'JOIN' || mode === 'CREATE') && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                            ...
                            {mode === 'CREATE' ? (
                                ...
                                >
                                    {nickname ? 'ZALOŽIT HRU' : 'ZADEJ SVOJE JMÉNO'}
                                </button>
                            ) : (
                                <button
                                    onClick={handleJoin}
                            ...
                            <div className="text-center mt-4">
                                <p className={`text-xs font-mono font-bold ${connected ? 'text-green-500' : 'text-red-500'}`}>
                                    Status: {connected ? '🟢 PŘIPOJENO' : '🔴 ODPOJENO'} {socket?.id ? `(${socket.id.substring(0, 4)})` : ''}
                                    {connected && <button onClick={handlePing} className="ml-2 underline opacity-50 hover:opacity-100 cursor-pointer">Test Ping</button>}
                                </p>
                                {!connected && <p className="text-[10px] text-white/30 mt-1">Probíhá pokus o spojení...</p>}
                            </div>

                            <div className="mt-4 text-[10px] font-mono text-left w-full h-32 overflow-y-auto bg-black/80 rounded border border-white/10 p-2 text-green-400 pointer-events-none opacity-80">
                                {debugLogs.length === 0 && <span className="opacity-50 italic">Tady se objeví diagnostika...</span>}
